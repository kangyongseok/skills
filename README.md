...
개인 FE 에이전트 팩(`~/fe-agent-pack`)이 삭제되어 아래 두 스킬은 심링크가 깨진 상태이며 파일을 복구할 수 없어 포함하지 못했습니다.

| 스킬 | 상태 | 원래 위치 |
|------|------|-----------|
| `fe-review-checklist` | ⚠️ 심링크 깨짐 (파일 없음) | `~/fe-agent-pack/skills/fe-review-checklist` |
| `fe-scaffold` | ⚠️ 심링크 깨짐 (파일 없음) | `~/fe-agent-pack/skills/fe-scaffold` |

> 복구하려면 `~/fe-agent-pack` 백업 또는 원본 저장소를 찾아 다시 심링크하면 됩니다.
